  ...
  return (
    ...
    >
      <div className="flex flex-col gap-3 m-auto items-start p-8 min-w-[340px] sm:min-w-96 border rounded-xl text-sm text-zinc-600 shadow-lg">
        ...
        <button className="bg-primary text-white text-base w-full py-2 rounded-md">{state === 'sign up'?"Create Account":"Login"}</button>
      
      {
        state ==='sign up'?<p>Already have an account? <span onClick={()=>setState("login")} className="underline text-primary cursor-pointer">Login here</span></p>:<p>Create an new account? <span onClick={()=>setState('sign up')} className="underline text-primary cursor-pointer">Click here</span></p>
      }
      </div>
    </form>
  );
